...
export const MAX_OUTPUT_MESSAGES = 10;
const TRUNCATION_NOTICE = "\n\n[output truncated]";
const FENCE_CLOSE = "\n```";
type Fence = { opener: string };

export function splitOutput(
  value: string,
  limit = MESSAGE_LIMIT,
  maxChunks = MAX_OUTPUT_MESSAGES,
): string[] {
  if (limit < 32) throw new RangeError("Message limit must be at least 32 characters");
  if (maxChunks < 1) throw new RangeError("Message count must be at least 1");
  ...
  while (start < value.length && chunks.length < maxChunks) {
    ...
    const isLast = chunks.length === maxChunks - 1;
    const reserved = FENCE_CLOSE.length + (isLast ? TRUNCATION_NOTICE.length : 0);
    const { segment, next } = takeSegment(value, start, limit - prefix.length - reserved);
    const nextFence = updateFence(segment, fence);
    chunks.push(
      `${prefix}${segment}${nextFence ? FENCE_CLOSE : ""}${
        isLast && next < value.length ? TRUNCATION_NOTICE : ""
      }`,
    );
    start = next;
    fence = nextFence;
